...
# Print whether a secret is set without echoing its value
mask() {
  if [ -z "$1" ] || [ "$1" = "placeholder" ]; then
    echo "(not set)"
  else
...
echo "AZURE_API_KEY: $(mask "$AZURE_API_KEY")"
echo "ANTHROPIC_API_KEY: $(mask "$ANTHROPIC_API_KEY")"
echo "GROQ_API_KEY: $(mask "$GROQ_API_KEY")"
echo "COHERE_API_KEY: $(mask "$COHERE_API_KEY")"
echo "CO_API_KEY: $(mask "$CO_API_KEY")"
echo "HF_TOKEN: $(mask "$HF_TOKEN")"
echo "HUGGINGFACE_API_KEY: $(mask "$HUGGINGFACE_API_KEY")"
echo "DATABRICKS_API_KEY: $(mask "$DATABRICKS_API_KEY")"
echo "GEMINI_API_KEY: $(mask "$GEMINI_API_KEY")"
echo "CODESTRAL_API_KEY: $(mask "$CODESTRAL_API_KEY")"
echo "MISTRAL_API_KEY: $(mask "$MISTRAL_API_KEY")"
echo "AZURE_AI_API_KEY: $(mask "$AZURE_AI_API_KEY")"
echo "NVIDIA_NIM_API_KEY: $(mask "$NVIDIA_NIM_API_KEY")"
echo "XAI_API_KEY: $(mask "$XAI_API_KEY")"
echo "PERPLEXITYAI_API_KEY: $(mask "$PERPLEXITYAI_API_KEY")"
echo "GITHUB_API_KEY: $(mask "$GITHUB_API_KEY")"
echo "DEEPSEEK_API_KEY: $(mask "$DEEPSEEK_API_KEY")"
echo "AI21_API_KEY: $(mask "$AI21_API_KEY")"
echo "LANGSMITH_API_KEY: $(mask "$LANGSMITH_API_KEY")"
...
echo "CREATE_BEDROCK_AGENT_ENDPOINT: ${CREATE_BEDROCK_AGENT_ENDPOINT:-true}"
...
echo "LANGFUSE_SECRET_KEY: $(mask "$LANGFUSE_SECRET_KEY")"
...
export TF_VAR_memory_per_vcpu_mib=${ECS_MEMORY_PER_VCPU_MIB:-2048}
export TF_VAR_alb_idle_timeout_seconds=${ALB_IDLE_TIMEOUT_SECONDS:-600}
...
export TF_VAR_rds_deletion_protection=${RDS_DELETION_PROTECTION:-false}
export TF_VAR_rds_log_statement=${RDS_LOG_STATEMENT:-ddl}
